...
fn main() {
    ...
    if let Some(input) = cli.input {
        ...
        println!("");
        println!("注意：如需隐藏控制台窗口，请使用 hidden-build feature 编译：");
        println!("  cargo build --release --features hidden-build");
    }
}
...
fn extract_7z(archive_path: &str, output_dir: &str, password: Option<&str>) -> std::result::Result<(), String> {
    ...
    // 优先使用sevenz-rust2纯Rust解压（支持更多压缩方法，包括方法10）
    let decompress_result = if let Some(pwd) = password {
    ...
    match decompress_result {
        Ok(_) => return Ok(()),
        Err(e) => {
            // sevenz-rust2失败，尝试使用系统7z命令
            let rust_error = format!("{:?}", e);
            
            let seven_z_paths = vec![
                "C:\\Program Files\\7-Zip\\7z.exe",
                "C:\\Program Files (x86)\\7-Zip\\7z.exe",
                "X:\\Windows\\System32\\7z.exe",
                "X:\\Program Files\\7-Zip\\7z.exe",
                "7z.exe",
                "7za.exe",
            ];
            
            for seven_z_path in &seven_z_paths {
                let is_available = if Path::new(seven_z_path).exists() {
                    true
                } else {
                    Command::new(seven_z_path)
                        .arg("--help")
                        .output()
                ...
                if is_available {
                    let mut cmd = Command::new(seven_z_path);
                    cmd.arg("x")
                       .arg(archive_path)
            ...
            return Err(format!("解压失败: {}", rust_error));
        }
    }
}
...
fn run_pecmd(script_path: &str) {
    ...
    let pecmd_exe = "X:\\Windows\\PECMD.EXE";
    
    // 根据进程是否存在选择命令
    let cmd_arg = if is_running { "LOAD" } else { "MAIN" };
    
    // 直接调用PECMD.EXE，不使用cmd /c
    let mut command = Command::new(pecmd_exe);
    command.arg(cmd_arg);
    command.arg(script_path);
    
    // 设置工作目录为X:\Windows
    command.current_dir("X:\\Windows");
...
fn load_edgeless_plugins() {
    ...
    for drive in &drives {
        ...
        if Path::new(&search_path).exists() {
            edgeless_drive = drive.clone();  // 包含冒号，如"C:"
            break;
        }
    }
    ...
    // 并行处理插件
    let handles: Vec<_> = plugin_files.into_iter().map(|plugin_path| {
        thread::spawn(move || {
            println!("  处理插件: {}", plugin_path);
            load_specific_edgeless_plugin(&plugin_path, true);
        })
    }).collect();
...
fn load_hpm_modules() {
    ...
    for drive in &drives {
        ...
        if Path::new(&search_path).exists() {
            hpm_drive = drive.clone();  // 包含冒号，如"C:"
            break;
        }
    }
        ...
        for entry in entries.flatten() {
            ...
            if let Some(ext) = path.extension() {
                if ext.to_str().unwrap().eq_ignore_ascii_case("hpm") {
                    if let Some(path_str) = path.to_str() {
                        module_files.push(path_str.to_string());
                    }
                }
            }
        }
    ...
    // 并行处理模块
    let handles: Vec<_> = module_files.into_iter().map(|module_path| {
        thread::spawn(move || {
            println!("  处理模块: {}", module_path);
...
fn process_hpm_module(module_path: &str) {
    // 使用随机后缀创建唯一的临时目录，避免并行处理冲突
    let tmp_suffix = generate_random_string(8);
    let tmp_dir = format!("X:\\Program Files\\HotPEModules\\tmp_{}", tmp_suffix);
    ...
    match extract_7z(module_path, &tmp_dir, None) {
        Ok(_) => {
            ...
            // 读取模块名称（HPM.config是ANSI/GBK编码）
            let config_path = format!("{}\\HPM.config", tmp_dir);
            let mut mod_name = "unknown".to_string();
            
            if let Ok(bytes) = fs::read(&config_path) {
                // 使用encoding_rs将GBK编码转换为UTF-8
                let (decoded, _, had_errors) = encoding_rs::GBK.decode(&bytes);
                if !had_errors {
                    // 创建临时UTF-8文件供INI解析
                    let temp_config = format!("{}\\HPM.config.utf8", tmp_dir);
                    if fs::write(&temp_config, decoded.as_bytes()).is_ok() {
                        if let Ok(conf) = Ini::load_from_file(&temp_config) {
